...
inline void online_exp_sum_max_simd(const float* src, size_t n, float* out_max, float* out_sum){
    ...
    __m256 v_d = _mm256_set1_ps(1.0f); // exp(src - src) = 1.0f for all 8 lanes
    ...
    for(; i + 8 <= n; i += 8){
        ...
        __m256 v_correction_exp = _mm256_sub_ps(v_m_prev, v_m);
        __m256 v_element_exp = _mm256_sub_ps(v_src, v_m);
        
        __m256 v_correction_factor = avx2_exp_ps(v_correction_exp);
        __m256 v_new_elements = avx2_exp_ps(v_element_exp);
